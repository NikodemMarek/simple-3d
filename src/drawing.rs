...
pub fn render_triangle(camera: &mut Screen, Triangle(a, b, c): Triangle<2>, pixel: Pixel) {
    #[inline]
    ...
    for x in lb..rb {
        for y in tb..bb {
            ...
            if det(a, b, p) >= 0.0 && det(b, c, p) >= 0.0 && det(c, a, p) >= 0.0 {
                draw_point(camera, (x as u32, y as u32), pixel);
            }
        }
    }
}
fn draw_point(camera: &mut Screen, (x, y): (u32, u32), pixel: Pixel) {
    let index = (x + y * camera.width) as usize;
    if index < camera.buffer.len() {
        camera.buffer[index] = pixel;
    }
}
